                    ...
                    Int Offset )
{
    ...
    {
        MPI_Isend(&ScalarField[0][0],       1, BottomTop, BottomID, 0, MPI_COMM_WORLD, &request[comm]);
        MPI_Irecv(&ScalarField[0][-Offset], 1, BottomTop, BottomID, 2, MPI_COMM_WORLD, &request[comm+1]);
    ...
    {
        MPI_Isend(&ScalarField[0][Ny-Offset], 1, BottomTop, TopID, 2, MPI_COMM_WORLD, &request[comm]);
        MPI_Irecv(&ScalarField[0][Ny],        1, BottomTop, TopID, 0, MPI_COMM_WORLD, &request[comm+1]);
    ...
    if (LeftID == Rank)
    {
        for (int i0 = 0; i0 < Offset; i0++)
            for (int i1 = 0; i1 < Ny; i1++)
                ScalarField[-i0-1][i1] = ScalarField[Nx-i0-1][i1];
    }
    else
    {
        MPI_Isend(&ScalarField[0][0],       1, LeftRight, LeftID, 3, MPI_COMM_WORLD, &request[comm]);
        MPI_Irecv(&ScalarField[-Offset][0], 1, LeftRight, LeftID, 1, MPI_COMM_WORLD, &request[comm+1]);
    ...
    {
        MPI_Isend(&ScalarField[Nx-Offset][0], 1, LeftRight, RightID, 1, MPI_COMM_WORLD, &request[comm]);
        MPI_Irecv(&ScalarField[Nx][0],        1, LeftRight, RightID, 3, MPI_COMM_WORLD, &request[comm+1]);
